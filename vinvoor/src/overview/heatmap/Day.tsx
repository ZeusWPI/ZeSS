            ...
            {isDayVariant(variant)
              ? Array.from({ length: DAYS_IN_WEEK }, (_, cidx) => {
                ...
                if (data.data[currentDate.getTime()])
                  ...
                  colors = theme.heatmap.colorUnreachable;

                let dataTooltipContent = ""
                if (currentDate > new Date())
                  dataTooltipContent = ""
                else if (data.data[currentDate.getTime()])
                  dataTooltipContent = `Present on ${DATE_FORMATTER.format(currentDate)}`
                else
                  dataTooltipContent = `Absent on ${DATE_FORMATTER.format(currentDate)}`
              ...
              : Array.from({ length: WEEKS_IN_MONTH }, (_, cidx) => {
                ...
                const dataTooltipContent = `${count} scan${count !== 1 ? "s" : ""
