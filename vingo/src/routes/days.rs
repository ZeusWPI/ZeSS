...
pub async fn get(state: State<AppState>) -> ResponseResult<Json<Vec<day::Model>>> {
    Ok(Json(Day::find()
        .all(&state.db)
        .await
        .or_log((StatusCode::INTERNAL_SERVER_ERROR, "failed to get days"))?))
}
...
pub async fn delete(state: State<AppState>, Path(day_id): Path<i32>) -> ResponseResult<()> {
    Day::delete_by_id(day_id).exec(&state.db).await.or_log((StatusCode::INTERNAL_SERVER_ERROR, "failed to delete day"))?;
    Ok(())
}
